...
import styles from "./CreateLaneModal.module.css";
...
export function CreateLaneModal({
  ...
  const handleSubmit = (e) => {
    ...
    // Check for duplicate lane names (case-insensitive)
    const normalizedName = laneName.trim().toLowerCase();
    const isDuplicate = existingLanes.some(
      (lane) => lane.title.toLowerCase() === normalizedName
    );
    ...
    <>
      ...
      <div className={styles.modal}>
        <div className={styles.header}>
          <h2 className={styles.title}>Create New Lane</h2>
          <button
